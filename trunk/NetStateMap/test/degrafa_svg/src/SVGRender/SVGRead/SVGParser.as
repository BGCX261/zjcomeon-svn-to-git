...
package SVGRender.SVGRead {
	...
	public class SVGParser {
		...
		private function visitPolygon(elt:XML):Object {
			var obj:Object = new Object();
			obj.points = obtenerPuntos(elt.@points);
		...
		public static function comprobar(s:String):String{
			var str:Array=s.split("e");
			if(str.length>1)
				return String(Math.pow(Number(str[0]) ,   Number( str[1] ) ) );			
			return s;
		}
		...
		public static function parseArgsData(input:String):Array {
			...
			input = StringUtil.replace(input,"\t","");
			input = StringUtil.shrinkSequencesOf(input,",");
			input = StringUtil.trim(input, ",");
			
			var argsArray:Array = input.split(",");			
			for (var i:int=0; i<argsArray.length; i++) { 
				if(argsArray[i].length>0){
					var s:String=comprobar(argsArray[i]);								
					returnData.push(Number(s));
				}
			}
			return (returnData); 
		}
		...
		public static function obtenerPuntos(input:String):Array {
			...
			var puntosArray:Array=input.split(" ");
			for each (var parPuntos:String in puntosArray){
				var par:Array=parPuntos.split(",");				
				if(par[0]!=null && par[1]!=null){
					//trace("coordenada: " + par[0] + "," + par[1]);
					var p:Point=new Point(par[0], par[1]);
					returnData.push(p);	
				}
		...
		public function parseTransformation(m:String):Matrix {
			...
			for each(var att:String in att_array){
				...
				//trace("String de puntos, MATRIX: " + att);
		...
		private function parseGradient(id:String):Object{
			...
			for each(var stop:XML in xml_grad.*::stop){
				...
				grad.ratios.push( offset );
			}

			//Se guarda la definicion de gradientes
