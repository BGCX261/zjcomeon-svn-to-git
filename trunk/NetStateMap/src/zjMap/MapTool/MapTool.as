package zjMap.MapTool
{
	...
	public class MapTool extends UIComponent
	{
		 ...
		 * @conn:		标记连接锁
		 * */
		public function MapTool(mapScale:Number = 1, mapMaxScale:Number = 2, lock:Boolean = true, move:Boolean = true, conn:Boolean = true)
		{
			super();
		
			zVSliderValue = mapScale * 10;		/**根据组件大小设置比率*/
			zVSliderMaxVal = mapMaxScale * 10;	/**设置上限比率*/
			...
			if(evt.target.hasOwnProperty("lockState")){
				...
				if(toolBtnGroupSelected != undefined && toolBtnGroupSelected == evt.target){
					...
					if(toolBtnGroupSelected.lockState){
						...
						toolBtnGroupSelected.lockState = true;
					}
				}else{
		 ...
		 * 设置标记移动开关值
		 * */
		public function setMConnectAble(value:Boolean):void{
			//zConnect.setLockStateIcon(tf);
			zConnect.lockState = value;
		}
